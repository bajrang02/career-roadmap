...
import { listSkills, SKILL_COUNT, SKILL_CATEGORIES } from "@/lib/data-loader";
...
function SkillsInner() {
  ...
  const categories = useMemo(() => {
    ...
    const order = new Map(SKILL_CATEGORIES.map((c) => [c.label, true]));
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-xl body-text">
            {SKILL_COUNT} structured skill roadmaps — languages, frameworks, tools and platforms —
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {[{ id: "all", count: all.length }, ...categories].map((c) => (
          <button
            key={c.id}
