...
import { listRoadmaps, SKILL_COUNT, TOTAL_TOPICS } from "@/lib/data-loader";
...
function RoadmapsInner() {
  const all = useMemo(() => listRoadmaps(), []);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="eyebrow">
            {all.length} roadmaps · {careerCount} careers · {SKILL_COUNT} skills ·{" "}
            {TOTAL_TOPICS.toLocaleString("en-US")} topics
          </p>
        ...
        <div className="flex rounded-full border border-slate-200 bg-white p-0.5 dark:border-slate-700 dark:bg-slate-900">
          {(
            [
              ...
              { key: "skill", label: "Skills", count: SKILL_COUNT },
